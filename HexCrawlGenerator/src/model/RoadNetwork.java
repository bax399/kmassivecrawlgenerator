		...
		for (Connection cc : pathconnects) {
			...
			for (int ii = 0; ii < 2; ii++) {
				...
				if (hexes.add(fh)) {
					...
					{
						if (!nodes.contains(fh.getRoadNode()))
							tempset = joinNetworks(fh.getRoadNode().getNetwork(), tempset);
					}
 ...
 * Future stuff
 * 
 * //check if a crossing of road->river exists at this hex public boolean
 * getCrossing(FilledHex origin) {
 * 
 * return false; }
 * 
 * public int getIntersectionNumber(FilledHex origin) { //TODO change this to
 * accessing the hex's RoadNodes, and counting its size. //return number of
 * connections with this hex int count=0; Iterator<Connection> it =
 * roads.iterator(); while(it.hasNext()) { if (it.next().containsHex(origin)) {
 * count++; } } return count; }
 * 
 * public Set<Town> getStructuresInHex(FilledHex origin) { Iterator<Town> it =
 * towns.iterator(); Set<Town> townsFound = new HashSet<>(); while(it.hasNext())
 * { Town ss = it.next(); if (origin.contains(ss)) { townsFound.add(ss); } }
 * 
 * return townsFound; }
 * 
 * public Set<Town> getStructuresAlongRoad(FilledHex origin, Connection
 * direction) { //Follow connection, whenever it splits, send recursive call to
 * follow //Any
 * 
 * //if it only has one, return all towns in the set except the ones in the
 * origin hex. return null; }
 * 
 * 
 * //OLD STUFF public void initialRoad(ConnectedHexMap chm, Set<Connection>
 * path) { connects = path; Set<Connection> tempset = new HashSet<>();
 * tempset.addAll(connects); //add all hexes to set (get from connections)
 * for(Connection cc : connects) { //create river nodes and add to set,
 * FilledHex fh;
 * 
 * //if added to the set, create a river node at a random point for(int ii=0;
 * ii<2; ii++) { fh = cc.getVertexes().get(ii); if(hexes.add(fh)) {
 * if(fh.getRoadNode()==null) //No roads currently on it. { RoadNode rn; if
 * (fh.getLargestTown() !=null) { rn = new RoadNode(this,
 * fh.getLargestTown().getPosition()); } else { rn = new RoadNode(this,
 * chm.getRandomPoint(fh)); } fh.add(rn); //add road node to the hex
 * 
 * nodes.add(rn); }
 * 
 * if (fh.getTowns().size() > 0) { towns.addAll(fh.getTowns()); } } } }
 * 
 * connects = tempset;
 * 
 * }
